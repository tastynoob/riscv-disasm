...
void disassembler_t::add_instructions(const uint64_t xlen)
{
...
#define DEFINE_SFENCE_TYPE(code) add_sfence_insn(this, #code, match_##code, mask_##code);
    ...
    if (false) {
        ...
        for (size_t elt = 0; elt <= 7; ++elt) {
            ...
            uint64_t elt_map[] = { 0x00000000, 0x00005000, 0x00006000, 0x00007000,
            ...
            for (uint64_t i = 0, nf = 7; i < 4; i++, nf >>= 1) {
...
disassembler_t::disassembler_t(const uint64_t xlen)
{
    // highest priority: instructions explicitly enabled
    add_instructions(xlen);
